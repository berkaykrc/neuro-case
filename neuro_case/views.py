from flask import Blueprint, render_template, abort, current_app, request, flash
...
@main_page_bp.route('/search', methods=['GET'])
def search():
    if request.method == "GET":
        searched_location = request.args.get('search')
        print(searched_location)
        error = None
        if not searched_location:
            error = "Your geolocation is empty"
        if error is None:
            mkad_km = [
            ...
            mkad_area = {"left_bottom": (
                55.594760, 37.520957), "top_right": (55.880656, 37.725577)}
            params = {
                # "geocode": f'{lat},{long}',
                "apikey": current_app.config['SECRET_KEY'],
                "bbox": f'{mkad_area["left_bottom"]},{mkad_area["top_right"]}',
                "rspn": 1,
                "format": "json"
            }
            ...
            try:
                ...
                return render_template('main/search.html', json=json)
        flash(error)
    else:
